...
\brief
-	use the sub-controller string "KickSwitchController" for the view
view container that holds the knobs to link; there can only be one
KickSwitchController for each view container

-	To hard-wire the linkage, set the parameter in the constructor
	...
	*/
	void valueChanged(CControl* control) override
	{
		...
		if (control == activator)
		{
            ...
            if (isFloatEqual(control->getValueNormalized(), 1))
            {
	...
	\return the verified view
	*/
	CView* createView(const UIAttributes& attributes, const IUIDescription* description) override { return parentController->createView(attributes, description); }
